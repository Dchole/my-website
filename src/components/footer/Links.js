      ...
      {groups.map(({ title, group, align }) => (
        <nav key={align} className="mb-12">
          ...
          {align === "md:text-center" && (
            <div className="my-5">
              <h4 className="text-gray-800 dark:text-gray-200 hover:text-purple-700 dark:hover:text-purple-400 text-xl md:text-center font-headline font-semibold mb-2">
                Github
              </h4>
              <ul className="space-y-2">
                {github.map(({ url, name }) => (
